...
<body id="body1">
     ...
     <section id="section-awal" style="overflow: hidden; height: 100vh; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center;">
          <header id="fh5co-header" class="fh5co-cover mid" role="banner" style="background-image:url(images/rat/cover_rat2.jpg); display: flex; flex-direction: column; justify-content: center; align-items: center;" data-stellar-background-ratio="0.5">
     ...
     <script>
          $(document).ready(function() {
               setInterval(loopingStatus, 1000);
               loopingStatus();
               // Jalankan `loopingStatus` setiap 3 detik
          });

          function loopingStatus() {
               $.ajax({
                    url: "api/v1/pushbutton_kkm.php", // Ganti dengan URL API yang sesuai
                    type: "GET",
                    dataType: "json",
                    success: function(response) {
                         let value = response[0].txtParameterValue;
                         if (value === 'false') {
          ...
          $(".reset").click(function() {
               $.ajax({
                    url: "api/v1/resetpushbutton_kkm.php", // Ganti dengan URL API atau file JSON yang sesuai
                    type: "POST",
                    dataType: "json",
                    success: function(response) {
                         Swal.fire({
